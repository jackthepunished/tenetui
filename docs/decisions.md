...
## 2026-07-05 — Dual-pivot pincer map: a horizontal band, not two overlaid wells

**Context:** M6 follow-up. The single-pivot map is a radial gravity well around one playhead; the pincer has two. Overlaying two radial wells (each commit plotted relative to both) would be an unreadable tangle.
**Decision:** In pincer mode the map switches layout to a **horizontal spacetime band**: x is the commit's index across the full width (past left, future right), so the two playheads are two points *on the same axis*; y pinches toward whichever pivot a node is nearest (two wells along the band) via a shared `hash_scatter`; hue is the nearer pivot's color (red forward / blue inverted), so territory splits at the midpoint. Each pivot draws its own white-hot cluster + a red/blue dashed web to its nearest commits (`dashed_arc` and `pivot` were generalized to arbitrary targets/centers, adding the missing `t²` bezier term). Both decks' comet trails render. `paint()` branches to `paint_single`/`paint_pincer`; the details panel stays on the focused deck.
**Consequences:** The pincer's motion reads directly — the two anchors start together and slide apart as playback fans them, red growing rightward and blue leftward from the shared "now" — which is the single best marketing frame the tool produces. Still one color per braille cell, so where the two webs' tails cross they share a color; acceptable, and rarer on a horizontal band than in a radial tangle. Reuses every existing helper (grid, node, trails), so it added one layout function, not a subsystem.
